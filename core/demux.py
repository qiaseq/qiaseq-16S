...
_KMER_SIZE_ = 8
# First _SEED_SIZE_ bases from read sequence will be used for generating kmers and looking up primer k-mer index
_SEED_SIZE_ = 30
# Bases to pad to read sequence past the primer length when computing editdistance
_PADDING_ = 5
# editdist/len(primer); only primers >= to this value will be considered valid for trimming
_MISMATCH_THRESHOLD_ = 0.10
# max lines to read into memory (should be divisible by 4 for accurate parsing of fastq)
_CHUNK_SIZE_ = 40000000
...
def create_primer_search_datastruct(primer_file, cache=False, cache_file=None):
    ...
    datastruct = {
        "fwd_primer_kmer": forward_primer_kmer,
        "fwd_primer_info": forward_primers,
        "rev_primer_kmer": reverse_primer_kmer,
        "rev_primer_info": reverse_primers,
    }
    if cache:
...
def trim_primer(primer_datastruct, read):
    ...
    candidates = set()
    for oligo in set(
        "".join(itertools.islice(read[0:_SEED_SIZE_], i, i + _KMER_SIZE_))
        for i in range(len(read[0:_SEED_SIZE_]) + 1 - _KMER_SIZE_)
    ):  # all 8-mers of the first 30 bp of the read(length will be truncated to read length if read is less than 30 bp)
    ...
    for p in candidates:
        ...
        if read[0:p_len] == p and num_candidates == 1:  # exact match and no other primers to check
            ...
            padded_pos = p_len + _PADDING_
            alignment = edlib.align(p, read[0:padded_pos], mode="SHW")
            editdist = alignment["editDistance"]
            score = float(editdist) / p_len
            if best_score is None or score <= best_score:
                if score == best_score:
                    to_log[best_primer] = (
                        "*** Primers have same score *** Score : {score} : Primer1 : {p1} ; Primer2 : {p2} Read : {read}".format(
                            score=score, p1=best_primer, p2=p, read=read
                        )
                    )

                if (
                    best_plen is None or best_plen < p_len
                ):  # when same score with same primer length; the first hit will be chosen
                    best_alignment = alignment
                    best_primer = p
                    best_score = score
                    best_editdist = editdist

    assert best_score is not None, "Bug in logic ! Primer could not be scored correctly !"
...
def wrapper_trimmer(primer_datastruct, read_info):
    ...
    R1_trim_pos, R1_primer, R1_primer_err, R1_amplicon, R1_primer_name = trim_primer(
        (primer_datastruct["fwd_primer_kmer"], primer_datastruct["fwd_primer_info"]), R1_seq
    )
    R2_trim_pos, R2_primer, R2_primer_err, R2_amplicon, R2_primer_name = trim_primer(
        (primer_datastruct["rev_primer_kmer"], primer_datastruct["rev_primer_info"]), R2_seq
    )

    return (
        (R1_id, R1_seq, R1_t, R1_qual, R1_trim_pos, R1_primer, R1_primer_err, R1_amplicon, R1_primer_name),
...
def main(
        ...
        for R1_info, R2_info in p.map(func, chunks):
            ...
            if R1_trim_pos == -1 or R2_trim_pos == -1:  # No primer found
                ...
                if primer_3_bases == -1 or primer_3_bases > len(R1_seq):  # keep R1 and R1_qual to be as is
                    ...
                    adj_pos = R1_trim_pos - primer_3_bases
                    R1_seq = R1_seq[adj_pos:]
                    R1_qual = R1_qual[adj_pos:]
                ...
                if primer_3_bases == -1 or primer_3_bases > len(R2_seq):  # keep R2 and R2_qual to be as is
                    ...
                    adj_pos = R2_trim_pos - primer_3_bases
                    R2_seq = R2_seq[adj_pos:]
                    R2_qual = R2_qual[adj_pos:]
            ...
            if outsuffix not in OUT_R1:  # initialize file handle
                OUT_R1[outsuffix] = open(
                    os.path.join(output_dir, sample_name + "_" + outsuffix + "_L001_R1_001.fastq"), "w"
                )
                OUT_R2[outsuffix] = open(
                    os.path.join(output_dir, sample_name + "_" + outsuffix + "_L001_R2_001.fastq"), "w"
                )
            OUT_R1[outsuffix].write(
                "{r_id}\n{r_seq}\n{plus}\n{r_qual}\n".format(r_id=R1_id, r_seq=R1_seq, plus=R1_t, r_qual=R1_qual)
            )
            OUT_R2[outsuffix].write(
    ...
    with open(os.path.join(output_dir, "{sample_name}.primer_counts.txt".format(sample_name=sample_name)), "w") as OUT:
        ...
        for primer_name in fwd_primer_names:
            ...
            OUT.write(
                "{amplicon}\t{primer_name}\t{primer_seq}\t{count}\n".format(
                    amplicon=amplicon, primer_name=primer_name, primer_seq=primer_seq, count=count
                )
            )
    ...
    with open(os.path.join(output_dir, "{sample_name}.metrics.txt".format(sample_name=sample_name)), "w") as OUT:
        for met in [
            "read fragments, total",
            "read fragments dropped, primer not found",
            "read fragments dropped, R1 and R2 not on same amplicon",
        ]:
