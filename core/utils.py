...
def aggregate_metrics(output_dir, samples_cfg):
    ...
    for sample_name, R1_fastq, R2_fastq in parse_config_file(samples_cfg):
        sample_dir = os.path.join(output_dir, sample_name)
        read_metric_files[sample_name] = os.path.join(
            sample_dir, "{sample_name}.metrics.txt".format(sample_name=sample_name)
        )
        primer_metric_files[sample_name] = os.path.join(
            sample_dir, "{sample_name}.primer_counts.txt".format(sample_name=sample_name)
        )
